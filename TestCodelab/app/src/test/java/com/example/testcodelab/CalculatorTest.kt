...
class CalculatorTest {
    ...
    fun subTwoNumbersInfinity() {
        val resultAdd = mCalculator.sub(1.0, Double.POSITIVE_INFINITY)
        assertThat(resultAdd, `is`(equalTo(Double.NEGATIVE_INFINITY)))
    }

    @Test
    fun subTwoNumbersZero() {
        val resultAdd = mCalculator.sub(1.0, 0.0)
        assertThat(resultAdd, `is`(equalTo(1.0)))
    }

    @Test
    fun subTwoNumbersLarge() {
        val resultAdd = mCalculator.sub(10.0.pow(magnitude), 10.0.pow(magnitude))
        assertThat(resultAdd, `is`(equalTo(0.0)))
    }
